 ...
 * $Revision: 4 $
 * $Date: 14/03/06 3:51p $
 * @brief    NANO1X2 series ADC driver source file
 *
 * @note
...
void ADC_EnableHWTrigger(ADC_T *adc,
                         ...
                         uint32_t u32Param)
{
    ADC->CR &= ~(ADC_CR_TRGS_Msk | ADC_CR_TRGCOND_Msk | ADC_CR_TRGEN_Msk);
    if(u32Source == ADC_TRIGGER_BY_EXT_PIN) {
        ADC->CR |= u32Source | u32Param | ADC_CR_TRGEN_Msk;
    } else {
        ADC->CR |= u32Source | ADC_CR_TRGEN_Msk;
    }
                            ...
                            uint32_t u32SampleTime)
{
    ...
    if (u32ChNum < 8)
        ADC->SMPLCNT0 = (ADC->SMPLCNT0 & ~(ADC_SMPLCNT_CH0SAMPCNT_Msk << (u32SampleTime * 4))) | (u32SampleTime << (u32ChNum * 4));
    else
        ADC->SMPLCNT1 = (ADC->SMPLCNT1 & ~ADC_SMPLCNT_INTCHSAMPCNT_Msk) | (u32SampleTime << ADC_SMPLCNT_INTCHSAMPCNT_Pos);
}
