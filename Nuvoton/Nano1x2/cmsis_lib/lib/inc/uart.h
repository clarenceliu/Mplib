 ...
 * $Revision: 4 $
 * $Date: 13/12/16 4:27p $
 * @brief    Nano1X2 Series uart control header file.
 *
 * @note
...
/*---------------------------------------------------------------------------------------------------------*/
#define UART_WORD_LEN_5        (0) /*!< UA_TLCTL setting to set UART word length to 5 bits */
#define UART_WORD_LEN_6        (1) /*!< UA_TLCTL setting to set UART word length to 6 bits */
#define UART_WORD_LEN_7        (2) /*!< UA_TLCTL setting to set UART word length to 7 bits */
#define UART_WORD_LEN_8        (3) /*!< UA_TLCTL setting to set UART word length to 8 bits */

#define UART_PARITY_NONE    (0x0 << UART_TLCTL_PBE_Pos) /*!< UA_TLCTL setting to set UART as no parity   */
#define UART_PARITY_ODD     (0x1 << UART_TLCTL_PBE_Pos) /*!< UA_TLCTL setting to set UART as odd parity  */
#define UART_PARITY_EVEN    (0x3 << UART_TLCTL_PBE_Pos) /*!< UA_TLCTL setting to set UART as even parity */
#define UART_PARITY_MARK    (0x5 << UART_TLCTL_PBE_Pos) /*!< UA_TLCTL setting to keep parity bit as '1'  */
#define UART_PARITY_SPACE   (0x7 << UART_TLCTL_PBE_Pos) /*!< UA_TLCTL setting to keep parity bit as '0'  */

#define UART_STOP_BIT_1     (0x0 << UART_TLCTL_NSB_Pos) /*!< UA_TLCTL setting for one stop bit  */
#define UART_STOP_BIT_1_5   (0x1 << UART_TLCTL_NSB_Pos) /*!< UA_TLCTL setting for 1.5 stop bit when 5-bit word length  */
#define UART_STOP_BIT_2     (0x1 << UART_TLCTL_NSB_Pos) /*!< UA_TLCTL setting for two stop bit when 6, 7, 8-bit word length */
