...
/* Includes ------------------------------------------------------------------*/
#include "tmpm36b_enc.h"

#if defined(__TMPM36B_ENC_H)
...
void ENC_Init(TSB_EN_TypeDef * ENx, ENC_InitTypeDef * InitStruct)
{
    ...
    tmp |=
        (InitStruct->PulseDivFactor) | (InitStruct->FilterValue << 4U) |
        (InitStruct->CompareStatus << 8U) | (InitStruct->IntEn << 3U);
    ENx->TNCR = tmp;
}
