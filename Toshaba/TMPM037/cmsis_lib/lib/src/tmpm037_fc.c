...
#define SR_BUSY_MASK              ((uint32_t)0x00000001)
#define SR_BUSY_FLAG              ((uint32_t)0x00000000)

#define PSRA_BLOCK0_MASK         ((uint32_t)0x00000001)
#define PSRA_BLOCK1_MASK         ((uint32_t)0x00000002)
#define PSRA_BLOCK2_MASK         ((uint32_t)0x00000004)
#define PSRA_BLOCK3_MASK         ((uint32_t)0x00000008)

#define PSRA_BLOCK_FLAG          ((uint32_t)0x00000000)
...
FunctionalState FC_GetBlockProtectState(uint8_t BlockNum)
{
    ...
    case FC_BLOCK_1:
        ...
        break;
    case FC_BLOCK_2:
        tmp = TSB_FC->PSRA & PSRA_BLOCK2_MASK;
        break;
    case FC_BLOCK_3:
        tmp = TSB_FC->PSRA & PSRA_BLOCK3_MASK;
        break;
    default:
        /* Do nothing */
        break;
...
FC_Result FC_EraseBlock(uint32_t BlockAddr)
{
    ...
    volatile uint32_t *addr1 = (uint32_t *) (FLASH_START_ADDR + FC_CMD_BC1_ADDR);
    volatile uint32_t *addr2 = (uint32_t *) (FLASH_START_ADDR + FC_CMD_BC2_ADDR);
    volatile uint32_t *BA = (uint32_t *) (FC_BLOCK_ADDR_MASK & BlockAddr);
...
FC_Result FC_EraseChip(void)
{
    ...
    for (BlockNum = FC_BLOCK_0; BlockNum <= FC_BLOCK_MAX; ++BlockNum) {
