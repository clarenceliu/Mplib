...
#define CR_IOC_MASK            ((uint32_t)0x000000FE)
#define CR_SCLKS_MASK          ((uint32_t)0x000000FD)
...
#define TFC_4B_TIL_CLEAR       ((uint32_t)0x000000FC)
#define RFC_RFIS_CLEAR         ((uint32_t)0x000000BF)

#define TFC_TFIS_CLEAR         ((uint32_t)0x000000BF)
...
void SIO_Init(TSB_SC_TypeDef * SIOx, uint32_t IOClkSel, SIO_InitTypeDef * InitStruct)
{
    ...
    tmp = SIOx->CR;
    tmp &= (CR_IOC_MASK & CR_SCLKS_MASK);
    tmp |= (IOClkSel | InitStruct->InputClkEdge);
    SIOx->CR = tmp;
    ...
    /* Set the interval time that valid only for SCLK output mode and double 
       buffer is enabled */
    if ((IOClkSel == SIO_CLK_BAUDRATE) && (InitStruct->DoubleBuffer == SIO_WBUF_ENABLE)) {
        tmp &= MOD1_SINT_MASK;
...
#endif                          /* defined(__TMPM36B_UART_H)  */
