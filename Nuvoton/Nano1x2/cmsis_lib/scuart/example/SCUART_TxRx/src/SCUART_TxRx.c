 ...
 * $Revision: 3 $
 * $Date: 13/12/16 4:27p $
 * @brief    Smartcard UART mode demo for Nano1x2 series MCU
 *
 * @note
...
int SCUART_TxRx(void)
{
    ...
    printf("Please connect SC0 CLK pin(PC.4) with SC0 I/O pin(PC.6)\n");
...
void SysInit(void)
{
    ...
    /* Set PC.4 and PC.6 pin for SC UART mode */
    SYS->PC_L_MFP &= ~(SYS_PC_L_MFP_PC4_MFP_Msk | SYS_PC_L_MFP_PC6_MFP_Msk);
    SYS->PC_L_MFP |= (SYS_PC_L_MFP_PC4_MFP_SC0_CLK | SYS_PC_L_MFP_PC6_MFP_SC0_DAT);
