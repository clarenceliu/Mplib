 ...
 * $Revision: 8 $
 * $Date: 14/03/27 1:46p $
 * @brief    NANO1X2 series PWM driver source file
 *
 * @note
                                  ...
                                  uint32_t u32DutyCycle)
{
    ...
    switch ((CLK->CLKSEL1 & (CLK_CLKSEL1_PWM0_CH01_S_Msk << (u32ChannelNum >> 1))) >> (CLK_CLKSEL1_PWM0_CH01_S_Pos >> (u32ChannelNum >> 1))) {
    case 0:
        u32PWM_Clock = __HXT;
        break;
    case 1:
...
{
    pwm->ADCTRGEN |= (PWM_ADTRGEN_ADTRGEN0_Msk << u32ChannelNum);
}
...
{
    pwm->ADCTRGSTS |= (PWM_ADCTRGSTS_ADTRGSTS0_Msk << u32ChannelNum);
}
...
{
    return ((pwm->ADCTRGSTS & (PWM_ADCTRGSTS_ADTRGSTS0_Msk << u32ChannelNum)) >> u32ChannelNum);
}
...
void PWM_EnableDeadZone (PWM_T *pwm, uint32_t u32ChannelNum, uint32_t u32Duration)
{
    ...
    // enable dead zone
    pwm->CTL |= (PWM_CTL_DZ01_Msk << u32ChannelNum);
}
...
{
    // write 1 clear
    pwm->INTSTS = (PWM_INTSTS_INTD0_Msk << u32ChannelNum);
}
...
{
    return ((pwm->INTSTS & (PWM_INTSTS_INTD0_Msk << u32ChannelNum)) ? 1 : 0);
}
